...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const xResult = (x1 - x2) ** 2;
  const yResult = (y1 - y2) ** 2;
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const vectorsLengths = (x1 ** 2 + y1 ** 2) * (x2 ** 2 + y2 ** 2);
  const angleCos = vectorsMult / vectorsLengths;
...
function getSumOfDigits(num) {
  ...
  const sum = Array.from(numToString).reduce((currentSum, currentNumber) => {
